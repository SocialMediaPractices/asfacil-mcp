...
const VALID_CROSSINGS = ['san_ysidro', 'pedwest', 'otay_mesa'] as const;
type CrossingId = typeof VALID_CROSSINGS[number];

const CROSSING_NAMES: Record<CrossingId, string> = {
  san_ysidro: 'San Ysidro Port of Entry',
  pedwest:    'PedWest (El Chaparral)',
  otay_mesa:  'Otay Mesa Port of Entry',
};

const CROSSING_HOURS: Record<CrossingId, string> = {
...
const TOOLS: Tool[] = [
  {
    name: 'get_wait_times',
    description:
      'Get live northbound (Mexico → USA) wait times at US-Mexico border crossings near San Diego / Tijuana. ' +
  ...
  },
  {
    name: 'predict_wait',
    description:
      'Predict what the wait time will be at a specific crossing in 1–6 hours from now. ' +
  ...
  },
  {
    name: 'get_history',
    description:
      'Get historical wait time data for a crossing — raw data points and pre-computed stats ' +
  ...
  },
  {
    name: 'get_best_times',
    description:
      'Get a day-of-week × hour-of-day wait time heatmap for a crossing. ' +
  ...
  },
  {
    name: 'get_southbound',
    description:
      'Get estimated southbound (USA → Mexico / Tijuana) wait times. ' +
...
async function handleGetWaitTimes(args: Record<string, unknown>) {
  const crossing = (args.crossing as string) ?? 'all';
  const params: Record<string, string> = {};
  if (crossing !== 'all') params.crossing = crossing;

  const data = await apiFetch('/api/v1/waittimes', params) as {
    fetched_at: string;
    data_source: string;
    crossings: Array<{
      id: string;
      ...
      is_open: boolean;
      last_updated: string;
      modes: Record<string, Record<string, { wait_minutes: number | null; lanes_open: number | null }>>;
    }>;
  };
...
async function handlePredictWait(args: Record<string, unknown>) {
  const crossing = args.crossing as string;
  if (!crossing || !VALID_CROSSINGS.includes(crossing as CrossingId)) {
    throw new Error(`crossing must be one of: ${VALID_CROSSINGS.join(', ')}`);
  }

  const params: Record<string, string> = {
    crossing,
    mode:        (args.mode        as string) ?? 'vehicle',
    lane:        (args.lane        as string) ?? 'general',
    hours_ahead: String(args.hours_ahead ?? 1),
  };
  if (args.origin) params.origin = args.origin as string;

  const data = await apiFetch('/api/v1/predict', params) as {
    crossing: string;
    mode: string;
    lane: string;
    hours_ahead: number;
    ...
    best_window?: { label: string; avg_minutes: number };
    trip?: { drive_minutes: number; total_minutes: number };
    recommendation?: { action: string; reason: string; savings_minutes?: number };
  };

  const crossingName = CROSSING_NAMES[crossing as CrossingId] ?? crossing;
  const lines: string[] = [
...
async function handleGetHistory(args: Record<string, unknown>) {
  const crossing = args.crossing as string;
  if (!crossing || !VALID_CROSSINGS.includes(crossing as CrossingId)) {
    throw new Error(`crossing must be one of: ${VALID_CROSSINGS.join(', ')}`);
  }
  ...
  const data = await apiFetch('/api/v1/history', params) as {
    crossing: string;
    mode: string;
    lane: string;
    ...
    data_points: Array<{ recorded_at: string; wait_minutes: number }>;
  };
...
async function handleGetBestTimes(args: Record<string, unknown>) {
  const crossing = args.crossing as string;
  if (!crossing || !VALID_CROSSINGS.includes(crossing as CrossingId)) {
    throw new Error(`crossing must be one of: ${VALID_CROSSINGS.join(', ')}`);
  }
  ...
  const data = await apiFetch('/api/v1/heatmap', params) as {
    crossing: string;
    mode: string;
    lane: string;
    days_analyzed: number;
    total_samples: number;
    overall_min_minutes: number | null;
    overall_max_minutes: number | null;
    best_slots: Array<{ dow_label: string; hour_label: string; avg_minutes: number }>;
    worst_slots: Array<{ dow_label: string; hour_label: string; avg_minutes: number }>;
    cells: Array<{ dow: number; dow_label: string; hour: number; hour_label: string; avg_minutes: number | null; sample_count: number }>;
  };
...
async function handleGetSouthbound(args: Record<string, unknown>) {
  const crossing = (args.crossing as string) ?? 'all';
  const res = await apiFetch('/api/southbound') as {
    crossings: Array<{
      crossingId: string;
  ...
  for (const c of crossings) {
    const id = c.crossingId as CrossingId;
    const mins = c.estimatedMinutes ?? c.avgWaitMinutes;
    const sourceLabel =
      c.dataSource === 'google_maps' ? 'Live Google Maps' :
    ...
    if (c.patternReason?.en) lines.push(`*${c.patternReason.en}*`);
    lines.push('');
  }
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
      case 'get_wait_times':   text = await handleGetWaitTimes(safeArgs);   break;
      case 'predict_wait':     text = await handlePredictWait(safeArgs);    break;
      case 'get_history':      text = await handleGetHistory(safeArgs);     break;
      case 'get_best_times':   text = await handleGetBestTimes(safeArgs);   break;
      case 'get_southbound':   text = await handleGetSouthbound(safeArgs);  break;
      default:
        return {

...
const CROSSING_NAMES: Record<string, string> = {
  san_ysidro: 'San Ysidro',
  pedwest:    'PedWest (El Chaparral)',
  otay_mesa:  'Otay Mesa',
};

const CROSSING_HOURS: Record<string, string> = {
  san_ysidro: '24h',
  pedwest:    '6am–2pm',
  otay_mesa:  '6am–10pm',
};
...
function waitColor(mins: number): string {
  ...
  if (mins <= 45)  return c.yellow;
  return c.red;
}

function bar(mins: number, max = 120): string {
...
async function fetchWaitTimes(crossing?: string) {
  const url = new URL(`${BASE}/api/v1/waittimes`);
  if (crossing) url.searchParams.set('crossing', crossing);
  if (KEY) {
...
async function main() {
  ...
  for (const crossing of data.crossings) {
    const id   = crossing.id as keyof typeof CROSSING_NAMES;
